...
/**
 * ─────────────────────────────────────────────────────────────────────────────
 * DateNavigator — El Diáfano
 * ─────────────────────────────────────────────────────────────────────────────
 * Componente de navegación temporal para el sidebar.
 *
 * INTEGRACIÓN CON URL:
 *   Lee y escribe el parámetro ?fecha=YYYY-MM-DD en la URL.
 *   Si no hay parámetro usa la fecha de hoy.
 *
 * USO BÁSICO:
 *   import DateNavigator from '@/components/DateNavigator';
 *   <DateNavigator />
 *
 * USO CON CALLBACK (si no usas URL params):
 *   <DateNavigator onDateChange={(fecha) => setFechaActual(fecha)} />
 *
 * PROPS:
 *   onDateChange?  (dateStr: string) => void   — callback al cambiar fecha
 *   minDate?       string YYYY-MM-DD           — límite inferior (default: '2024-01-01')
 *   activeDates?   string[]                    — fechas CON datos (puntito rojo)
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * QUERIES SUPABASE PARA LA PÁGINA QUE CONSUME ESTE COMPONENTE:
 * ─────────────────────────────────────────────────────────────────────────────
 *
 * // 1. Historias del día con noticias y medios
 ...
 *       id, titulo, sesgo_ia, imagen_url, link,
 *       medios ( nombre, sesgo_politico, logo_url )
 *     )
 *   `)
 ...
 *
 * // 2. Fechas activas (para los puntitos en el calendario)
 * //    Ejecutar UNA vez y cachear en el cliente.
 * const { data: fechasActivas } = await supabase
 *   .rpc('get_fechas_con_historias', { desde: '2024-01-01' });
 *   -- o bien:
 * const { data } = await supabase
 *   .from('historias')
 *   .select('fecha')
 *   .gte('fecha', '2024-01-01');
 * const activeDates = [...new Set(data.map(h => h.fecha.slice(0, 10)))];
 *
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ─── Helpers de fecha (sin bugs de timezone) ─────────────────────────────────
...
export default function DateNavigator({
  ...
  // Fecha seleccionada — inicializa desde URL o hoy
  ...
  return (
    <div style={s.root} ref={wrapperRef}>
      {/* Etiqueta */}
      <span style={s.label}>EDICIÓN</span>
      ...
      <div style={s.navRow}>
        ...
        <button style={s.pill} onClick={() => {
          ...
          <span style={s.pillDow}>{DIAS_SEMANA[selected.getDay()]}</span>
      ...
      {/* Badge "edición del archivo" */}
      {!isToday && (
        <div style={s.archiveBadge}>
          <span>◷</span>
          <span style={{ flex: 1 }}>Edición del archivo</span>
